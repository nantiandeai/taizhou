...
/**设为模板*/
function toactivitytpl(index){
	var row = WHdg.getRowData(index);
	//row.actvopttime = new Date(row.actvopttime).Format("yyyy-MM-dd hh:mm:ss");
	var params = {};
	if(row.actvsdate != 'undefined' && row.actvedate != 'undefined'){
	 params = $.extend({}, row,
			{
				actvsdate : new Date(row.actvsdate || null).Format("yyyy-MM-dd hh:mm:ss"),
				actvedate : new Date(row.actvedate || null).Format("yyyy-MM-dd hh:mm:ss"),
			});
	}
	delete params.actvopttime;
	$.messager.confirm('确认对话框', '您确认将所选择的设置为模板吗？', function(r){
		if (r){
			$.ajax({
				type: "POST",
				url: getFullUrl('/admin/activity/toActivitytpl'),
				data: params,
	...
	if (!r)return;
	 var params = _params="";
  	 for(var i=0; i<row.length; i++){		  
  		 ...
  		 _params=",";
  		 if(row[i].actvstate == 2){
 	 		$.messager.alert("提示", "有未发布状态的活动, 无需取消发布!");
 	 		return;
 	 	 }
  	}
	$.ajax({
	...
	if (!r)return;
	 var params = _params="";
	  for(var i=0; i<row.length; i++){  
	 	 params+=_params+row[i].actvid;
	 	 _params=",";
	 	 if(row[i].actvstate == 3){
	 		$.messager.alert("提示", "含有活动处于发布状态, 需先取消发布!");
	 		return;
	 	 }
	  }
	$.ajax({
...
function plFP() {
	 var row = $('#activityDiv').datagrid('getSelections');  
	 if(row.length<1){
		 $.messager.alert('提示', '请选中要发布的记录！');
		 return;
	 }
	$.messager.confirm("提示", "确定要发布所选吗？", function(r) {
	if (!r)return;
	 var params = _params="";
	   for(var i=0; i<row.length; i++){  
	  	 params+=_params+row[i].actvid;
	  	 _params=",";
	  	if(row[i].actvstate == 3){
 	 		$.messager.alert("提示", "发布状态的活动, 无需重复发布!");
 	 		return;
 	 	 }
	   }
	$.ajax({
...
function datagridLoad(){ 
    ...
    datagrid.datagrid({
        url :'./admin/activity/loadActivity?actvstate2=2',
...
function callback(idx){
	...
	var data={actvid : row.actvid,fromstate:2,tostate : 1};
	$.messager.confirm("提示", "确定要打回审核吗？", function(r) {
		if (!r)return;
		lookActivity(data);
	})
}
...
function goalong(idx){
	var row = WHdg.getRowData(idx);
	var data={actvid : row.actvid,fromstate:2,tostate : 3};
...
/**取消发布*/
function notgoalong(idx){
	var row = WHdg.getRowData(idx);
	var data={actvid : row.actvid,fromstate:3,tostate:2};
...
<!-- datagrid div -->
	...
	<div id="activityTool" class="grid-control-group" style="display: none">
		...
		<shiro:hasPermission name="${resourceid }:checkoff">
			<a href="javascript:void(0)" class="easyui-linkbutton" onclick="dhFP();" data-options="size:'small'">打回所选</a>
		</shiro:hasPermission>
		</div>
	<div>
		详情标题:
		...
		活动状态:
		<select id="ccstate" class="easyui-combobox" data-options="value:''" name="actvstate" style="width:150px;">   
	...
	<shiro:hasPermission name="${resourceid}:view">
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="plain:true" validKey="actvstate" validVal="3"  method="toactivitytpl">设为模板</a>
	</shiro:hasPermission>
	...
	<shiro:hasPermission name="${resourceid }:publish">
		<a href="javascript:void(0)" validKey="actvstate" validVal="2" method="goalong">发布</a> 
	</shiro:hasPermission>
	
	<shiro:hasPermission name="${resourceid }:publishoff">
		<a href="javascript:void(0)" validKey="actvstate" validVal="3" method="notgoalong">取消发布</a> 
	</shiro:hasPermission>
	
	<shiro:hasPermission name="${resourceid }:checkoff">
		<a href="javascript:void(0)" validKey="actvstate" validVal="2" method="callback">打回审核</a>
	</shiro:hasPermission>
...
<div id="frm" class="none" style="display: none" data-options="	maximized:true">
			...
			<div class="main">
				...
				<div class="row">
					...
					<div>
						<select class="easyui-combobox" name="actvisyp"  style="height:32px;width: 90%" 
							data-options="editable:false">
							<option value="1">是</option>
		    				<option value="0">否</option>
						</select>
