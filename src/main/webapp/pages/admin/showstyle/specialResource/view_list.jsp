...
<head>
    ...
    <c:when test="${type eq 'edit'}">
        <c:set var="pageTitle" value="台州特色资源编辑列表"></c:set>
    </c:when>
    <c:when test="${type eq 'check'}">
        <c:set var="pageTitle" value="台州特色资源审核列表"></c:set>
    </c:when>
    <c:when test="${type eq 'publish'}">
        <c:set var="pageTitle" value="台州特色资源发布列表"></c:set>
    </c:when>
...
<script>
    ...
    function edit(idx){
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/specialResource/view/add?id='+row.id);
    }
