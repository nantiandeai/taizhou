...
<head>
    ...
    <c:choose>
        <c:when test="${editType eq 'edit'}">
            <c:set var="pageTitle" value="文化联盟单位编辑"></c:set>
        </c:when>
        <c:when test="${editType eq 'add'}">
            <c:set var="pageTitle" value="文化联盟单位添加"></c:set>
        </c:when>
        <c:when test="${editType eq 'view'}">
            <c:set var="pageTitle" value="文化联盟单位查看"></c:set>
        </c:when>
        <c:otherwise>
...
<form id="whgff" method="post" class="whgff">
    ...
    <h2>${pageTitle}</h2>
    <input id="id" name="id" type="hidden" value="${whgCultureUnit.id}">
    <div class="whgff-row">
        <div class="whgff-row-label"><i>*</i>单位名：</div>
        <div class="whgff-row-input">
            <input class="easyui-textbox" name="unitname"
                   style="width: 500px; height: 32px" data-options="required:true,validType:['length[1,60]']" value="${whgCultureUnit.unitname}"/>
        </div>
    </div>
    <div class="whgff-row">
        <div class="whgff-row-label"><i>*</i>单位编码：</div>
        <div class="whgff-row-input">
            <input class="easyui-textbox" name="unitcode"
                   style="width: 500px; height: 32px" data-options="required:true,validType:['length[1,60]']" value="${whgCultureUnit.unitcode}"/>
        </div>
    </div>
    <div class="whgff-row">
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input" id="divInfolink">
            <input class="easyui-textbox" id="unitlink" name="unitlink"
                   style="width: 500px; height: 32px" data-options="required:true,validType:['length[1,1000]','url'],prompt:'请输入连接地址,如：http://yoursite.com/'" value="${whgCultureUnit.unitlink}"/>
        </div>
    </div>
</form>
