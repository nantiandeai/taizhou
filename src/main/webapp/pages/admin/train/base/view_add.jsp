...
<form id="whgff" method="post" class="whgff">
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input">
            <input class="easyui-combobox" name="venue" style="width:500px; height:32px" value="${whgTra.venue}"
                    data-options="editable:false, multiple:false, mode:'remote', valueField:'id', textField:'title', url:'${basePath}/admin/venue/srchList?state=6&delstate=0',
                    prompt:'请选择所属场馆',
    ...
    <div class="whgff-row">
        ...
        <div class="whgff-row-input">
            <input class="easyui-combobox" id="venroom" name="venroom" value="${whgTra.venroom}" style="width:500px; height:32px"
                    data-options="editable:false, multiple:false,valueField:'id', textField:'title', url:'${basePath}/admin/venue/room/srchList?state=6&delstate=0&venid=${whgTra.venue}',
                        prompt:'请选择活动室',
                        panelHeight:'auto',
                        editable:false,
                        loadFilter : function (data) {
    ...
    <div class="whgff-row train">
        ...
        <div class="whgff-row-input">
            <input class="easyui-datebox starttime" id="starttime" name="starttime" value="<fmt:formatDate value="${whgTra.starttime}" pattern="yyyy-MM-dd"/>" style="width:200px; height:32px" data-options="required:true"/>至
            <input class="easyui-datebox endtime" id="endtime" name="endtime" value="<fmt:formatDate value="${whgTra.endtime}" pattern="yyyy-MM-dd"/>" style="width:200px; height:32px" data-options="required:true, validType:'traEndTime[\'starttime\']'"/>
        </div>
    </div>
    ...
    <div class="whgff-row single" >
        ...
        <div class="whgff-row-input">
            <input class="easyui-datetimebox sin_starttime" name="sin_starttime" style="width:200px; height:32px" />至
            <input class="easyui-datetimebox sin_endtime" name="sin_endtime" style="width:200px; height:32px"/>
        </div>
    </div>
    ...
    <c:if test="${not empty course}">
        <c:forEach items="${course}" var="item" varStatus="s" end="0">
            <div class="whgff-row single_edit" >
                <div class="whgff-row-label"><label style="color: red">*</label>培训上课时间：</div>
                <div class="whgff-row-input">
                    <input class="easyui-datetimebox sin_starttime_edit" name="sin_starttime" value='<fmt:formatDate value="${item.starttime}" pattern="yyyy-MM-dd HH:mm:ss"/>' style="width:200px; height:32px" />至
                    <input class="easyui-datetimebox sin_endtime_edit" name="sin_endtime" value='<fmt:formatDate value="${item.endtime}" pattern="yyyy-MM-dd HH:mm:ss"/>' style="width:200px; height:32px"/>
                </div>
            </div>
        </c:forEach>
    </c:if>
    ...
    <c:if test="${not empty course}">
        <c:forEach items="${course}" var="item" varStatus="s">
            <div class="whgff-row multi_edit" name="multiContent">
                <div class="whgff-row-label"><label style="color: red">*</label>培训上课时间：</div>
                <div class="whgff-row-input">
                    <input class="easyui-datetimebox starttime_edit" id="_starttime_${s.count}" style="width:200px; height:32px" name="_starttime" value='<fmt:formatDate value="${item.starttime}" pattern="yyyy-MM-dd HH:mm:ss"/>' data-options="required:true,validType:'edittraTime[0,\'_starttime_${s.count}\']'">至
                    <input class="easyui-datetimebox endtime_edit" id="_endtime_${s.count}" style="width:200px; height:32px" name="_endtime" value='<fmt:formatDate value="${item.endtime}" pattern="yyyy-MM-dd HH:mm:ss"/>' data-options="required:true,validType:'edittraTime[1,\'_starttime_${s.count}\']'">
    ...
    $.extend($.fn.validatebox.defaults.rules, {
        ...
        traEndTime: {
            validator: function(value, param){
                ...
                var _enrollendtime = WhgComm.parseDateTime($("#whgff").find("input[name='enrollendtime']").val());
                return d2.getTime()>=d1.getTime() && d1.getTime() >= _enrollendtime.getTime();
            },
    ...
    function showTrainTime(isMulti){
        ...
        if(id){
            ...
            if(0 == isMulti){
                ...
                //多场添加用的上课时间表单
                $(".starttime_edit").datetimebox("disable");
                $(".endtime_edit").datetimebox("disable");
                if(state == 4){
            ...
            if(1 == isMulti){
                ...
                $(".starttime_edit").datetimebox("enable");
                $(".endtime_edit").datetimebox("enable");
                if(state == 4){
                    $('input[name="ismultisite"]').on('click', function(){return false});
                    $(".enrollstarttime").datetimebox('readonly');
                    $(".enrollendtime").datetimebox('readonly');
                    ...
                    $(".starttime_edit").datetimebox("readonly");
                    $(".endtime_edit").datetimebox("readonly");
                }
                ...
                $(".sin_starttime").datetimebox("disable");
                $(".sin_endtime").datetimebox("disable");
                $(".sin_starttime_edit").datetimebox("disable");
                $(".sin_endtime_edit").datetimebox("disable");
            }
            //固定场
            if(2 == isMulti){
                $(".train").css("display","");
