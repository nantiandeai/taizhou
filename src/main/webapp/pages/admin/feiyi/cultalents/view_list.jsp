...
<head>
    ...
    <c:when test="${type eq 'edit'}">
        <c:set var="pageTitle" value="文化人才编辑列表"></c:set>
    </c:when>
    <c:when test="${type eq 'check'}">
        <c:set var="pageTitle" value="文化人才审核列表"></c:set>
    </c:when>
    <c:when test="${type eq 'publish'}">
        <c:set var="pageTitle" value="文化人才发布列表"></c:set>
    </c:when>
...
<table id="whgdg" title="${pageTitle}" class="easyui-datagrid" style="display: none"
       data-options="fit:true, striped:true, rownumbers:true, fitColumns:true, singleSelect:false, checkOnSelect:true, selectOnCheck:true, pagination:true, toolbar:'#whgdg-tb', url:'${basePath}/admin/cultalents/srchList4p?type=${type}'">
    <thead>
    <tr>
...
<script>
     ...
     */
    function add(){
        WhgComm.editDialog('${basePath}/admin/cultalents/view/add');
    }
    ...
    function edit(idx){
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/cultalents/view/add?id='+row.id);
    }
    ...
    function view(idx){
        var row = $("#whgdg").datagrid("getRows")[idx];
        WhgComm.editDialog('${basePath}/admin/cultalents/view/add?targetShow=1&id='+row.id);
    }
    ...
    function del(idx){
        ...
        $.messager.confirm("确认信息", confireStr, function(r){
            if (r){
                $.messager.progress();
                $.post('${basePath}/admin/cultalents/del', {id: row.id}, function(data){
    ...
    function undel(idx){
        ...
        $.messager.confirm("确认信息", "确定要还原选中的项吗？", function(r){
            if (r){
                $.messager.progress();
                $.post('${basePath}/admin/cultalents/undel', {id: row.id, delstate: 0}, function(data){
                    $("#whgdg").datagrid('reload');
                    if (!data.success || data.success != "1"){
                        $.messager.alert("错误", data.errormsg||'操作失败', 'error');
                    }
    ...
    function __updStateSend(ids, formstates, tostate){
        ...
        $.post('${basePath}/admin/cultalents/updstate', params, function(data){
            $("#whgdg").datagrid('reload');
            if (!data.success || data.success != "1"){
                $.messager.alert("错误", data.errormsg||'操作失败', 'error');
            }
    ...
    function _updCommend(ids, fromState, toState){
        ...
        $.post('${basePath}/admin/cultalents/updCommend', params, function(data){
            $("#whgdg").datagrid('reload');
            if (!data.success || data.success != "1"){
                $.messager.alert("错误", data.errormsg||'操作失败', 'error');
            }
