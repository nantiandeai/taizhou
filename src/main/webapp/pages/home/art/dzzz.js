...
function loadArtContent(page, rows){
	...
	//导航条件
	var _url = getFullUrl('/art/srchdzzz?p=1');
	...
	//url
	if(arttyp != ''){
		_url += '&magearttyp='+arttyp;
	}
	if(condition != ''){
		_url += '&magetitle='+encodeURIComponent(condition);
	}
...
function showList(dataRows){
	...
	for(var i=0; i<dataRows.length; i++){
        _html += '<li>';
	    _html += '    <div class="img">';
	    _html += '        <a href="art/dzzzdetail?pagemageid='+dataRows[i].mageid+'" style="background-image:url('+getFullUrl(dataRows[i].magepic)+')">';
	    _html += '            <div class="mask"></div>';
	    _html += '        </a>';
	    _html += '    </div>';
	    _html += '    <p class="author">'+dataRows[i].magetitle+'</p>';
	    _html += '    <p class="pageCount">'+dataRows[i].magenum+'</p>';
	    _html += '</li>';
